...
import sys
output_file = "/xxxx/ab3_causal_disbaled_mucald.txt"
...
def tensor_only(x):
    if isinstance(x, (tuple, list)):
        if len(x) == 0:
            raise ValueError("Empty tuple/list passed where a tensor was expected.")
        return x[0]
    return x
...
class StandardNodeEncoder(nn.Module):
    def __init__(self, parents=None, node_dims=None):
        super().__init__()
        self.parents = parents
        self.node_dims = node_dims or []
        self.node_names = []
    ...
    def zprior(self, y_input):
        B = y_input.size(0)
        device = y_input.device
        mu_p_list = []
        lv_p_list = []
        for d in self.node_dims:
            mu_p_list.append(torch.zeros(B, d, device=device))
            lv_p_list.append(torch.zeros(B, d, device=device))
        return mu_p_list, lv_p_list
...
def train_local(loader, FE, SS, BE,
    ...
    for ep in range(LOCAL_EPOCHS):
        ...
        print(f"[Sched] ep={ep+1}/{LOCAL_EPOCHS} | lam_proxy={lam_proxy:.3g}, lam_diff={lam_diff:.3g}, lam_KLu={lam_klu:.3g}, lam_KLz={lam_klz:.3g}")
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            if global_exo1 is None:
                ...
                global_scm_template1 = StandardNodeEncoder(parents=parents, node_dims=node_dims).to(DEVICE)
                global_scm_template1.node_names = nodes
                global_scm_template2 = StandardNodeEncoder(parents=parents, node_dims=node_dims).to(DEVICE)
