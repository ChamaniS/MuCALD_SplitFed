# centralized_split_kvasir_fixed.py
import os
...
OUTPUT_DIR = "Outputs/test_preds_kvasir"
...
# Robust ComboLoss: handles index-targets or one-hot targets; uses multiclass Dice for 2-channel logits
class ComboLoss(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        self.num_classes = num_classes
        # multiclass works for C=2 as well (logits shape B,2,H,W)
    ...
    def forward(self, logits, targets):
        ...
        try:
            # if one-hot already and matches logits shape
            ...
            # common SMP API accepts integer target indices (B,H,W)
            if targets.dim() == 3:
            ...
            # fallback: convert indices -> one-hot
            if targets.dim() == 3:
                B, H, W = targets.shape
                C = logits.shape[1]
                t_idx = targets.long().to(device)
...
def evaluate_and_save_predictions(loader, FE, SS, BE, loss_fn, num_classes, save_dir, save_preds=True):
    # ensure eval mode for stable inference
            ...
            for bi in range(bsize):
                ...
                if basename is None and fname is not None:
                    if isinstance(fname, (list, tuple)):
                        ...
                        fullpath = fname
                if basename is None:
                    basename = f"kvasir_idx{idx*loader.batch_size + bi}.png"
                    fullpath = None
                ...
                # reconstruction metrics: recover image, resize to orig_size
                try:
                ...
                # per-sample reconstruction metrics for split1
                ...
                # per-sample reconstruction metrics for split2
    ...
    # finalize reconstruction metrics per split
...
def main():
    ...
    base = os.path.join(DATA_PATH, kvasir_SUBDIR)
    ...
    # loaders
    train_loader = get_loader(train_img_dir, train_mask_dir, CVCDataset, tr_tf, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=True)
    val_loader = get_loader(val_img_dir, val_mask_dir, CVCDataset, val_tf, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, shuffle=False)
    test_loader = get_loader(test_img_dir, test_mask_dir, CVCDataset, test_tf, batch_size=1, num_workers=NUM_WORKERS, shuffle=False)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(FE.state_dict(), os.path.join(BEST_MODELS_DIR, "best_FE_kvasir.pth"))
            torch.save(SS.state_dict(), os.path.join(BEST_MODELS_DIR, "best_SS_kvasir.pth"))
            torch.save(BE.state_dict(), os.path.join(BEST_MODELS_DIR, "best_BE_kvasir.pth"))
            print("Saved best kvasir models.")
