...
def main():
    ...
    for cname in client_dirs:
        ...
        client_train_loaders.append(get_loader(train_img, train_mask, CVCDataset, train_transform, shuffle=True))
        client_val_loaders.append(get_loader(val_img, val_mask, CVCDataset, val_transform, shuffle=False))  # no shuffle for val

    # Load mutual test set for global testing
    test_img = dataDir + "./test_imgs/"
    test_mask = dataDir + "./test_masks/"
    test_loader = get_loader(test_img, test_mask, CVCDataset, val_transform)
    ...
    for round in range(COMM_ROUNDS):
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_FE.state_dict(), "BestModels/best_FE_splitfed_kvasir.pth")
            torch.save(global_SS.state_dict(), "BestModels/best_SS_splitfed_kvasir.pth")
            torch.save(global_BE.state_dict(), "BestModels/best_BE_splitfed_kvasir.pth")
