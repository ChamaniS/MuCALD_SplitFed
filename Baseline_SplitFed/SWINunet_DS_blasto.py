...
def train_local(train_loader, val_loader, FE, SS, BE, denoiser1, denoiser2, optimizer_FE, optimizer_SS,optimizer_BE, loss_fn, client_id):
    FE.train()
    SS.train()
    BE.train()
    for epoch in range(LOCAL_EPOCHS):
        ...
        for data, targets in tqdm(train_loader, leave=False):
            ...
            if x1.dim() == 3:
                B, N, C = x1.shape
                H = W = int(N ** 0.5)
                x1 = x1.transpose(1, 2).contiguous().view(B, C, H, W)
            ...
            if denoised_x1.dim() == 4:
                B, C, H, W = denoised_x1.shape
                denoised_x1 = denoised_x1.view(B, C, H * W).transpose(1, 2).contiguous()

            x2 = SS(denoised_x1)
            if x2.dim() == 3:
                B, N, C = x2.shape
                H = W = int(N ** 0.5)
                x2 = x2.transpose(1, 2).contiguous().view(B, C, H, W)
...
def evaluate(loader, FE, SS, BE, denoiser1, denoiser2,loss_fn):
    ...
    with torch.no_grad():
        for data, targets in loader:
            ...
            if x1.dim() == 3:  # Swin output: [B, N, C]
                B, N, C = x1.shape
                H = W = int(N ** 0.5)  # assumes square grid of patches
                x1 = x1.transpose(1, 2).contiguous().view(B, C, H, W)  # → [B, C, H, W]

            noisy_x1 = forward_diffusion(x1, beta, alpha_cum, t)
            denoised_x1 = denoise(noisy_x1, denoiser1, alpha_cum, t, DEVICE)
            recon_loss1 = F.mse_loss(normalize_features(denoised_x1), normalize_features(x1))
            if denoised_x1.dim() == 4:  # [B, C, H, W]
                B, C, H, W = denoised_x1.shape
                denoised_x1 = denoised_x1.view(B, C, H * W).transpose(1, 2).contiguous()  # → [B, N, C]

            x2 = SS(denoised_x1)

            if x2.dim() == 3:  # Swin output: [B, N, C]
                B, N, C = x2.shape
                H = W = int(N ** 0.5)  # assumes square grid of patches
                x2 = x2.transpose(1, 2).contiguous().view(B, C, H, W)  # → [B, C, H, W]
...
def main():
    ...
    global_denoiser1 = initialize_denoiser(input_dim=96, output_dim=96, hidden_dim=128, device=DEVICE)
    global_denoiser2 = initialize_denoiser(input_dim=96, output_dim=96, hidden_dim=128, device=DEVICE)
    ...
    for round in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            train_local(client_train_loaders[i], client_val_loaders[i], FE, SS, BE, denoiser1, denoiser2,optimizer_FE, optimizer_SS,optimizer_BE,loss_fn, i+1)
            local_FE.append(FE)
            local_SS.append(SS)
            local_BE.append(BE)
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_FE.state_dict(), "BestModels/best_FE_swinUnet_DS_blasto.pth")
            torch.save(global_SS.state_dict(), "BestModels/best_SS_swinUnet_DS_blasto.pth")
            torch.save(global_BE.state_dict(), "BestModels/best_BE_swinUnet_DS_blasto.pth")
            torch.save(global_denoiser1.state_dict(), "BestModels/best_denoiser1_swinUnet_DS_blasto.pth")
            torch.save(global_denoiser2.state_dict(), "BestModels/best_denoiser2_swinUnet_DS_blasto.pth")
