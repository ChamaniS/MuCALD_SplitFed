...
LAMBDA_KLU = 1e-4
LAMBDA_KLZ = 1e-4
MU = 0.05  # fedprox coefficient
WARMUP_EPOCHS = 5
RAMP_EPOCHS = 5

LAMBDA_PROXY_TARGET = 0.40
LAMBDA_DIFF_TARGET = 0.20
...
def causal_invariant_fusion(
        ...
        eps=1e-8, min_fraction=0.05, blend_equal=0.3
):
    """
    Aggregate client models based on proxy MSE (causal alignment),
    dataset size, and optionally validation loss.

    Args:
        models: list of client models
        proxy_mse_scores: list of proxy MSEs (lower = better)
        client_sizes: dataset sizes per client
        val_losses: optional validation losses per client
        eps: small epsilon to prevent div/0
        min_fraction: minimum fraction of weight each client gets (e.g. 0.05)
        blend_equal: blending factor with equal weights (0.0 = no blend, 1.0 = all equal)
    ...
    # --- floor proxy MSE to avoid division explosion ---
...
def get_transforms(task_name):
    if task_name == "Blastocyst":  # microscopy embryo images
        return A.Compose([
            A.Resize(256, 256),
    ...
    elif task_name == "HAM10K":  # dermoscopy (skin lesions)
        return A.Compose([
            A.Resize(256, 256),
            A.HorizontalFlip(p=0.5),
    ...
    elif task_name == "Fetal":  # ultrasound
        return A.Compose([
            A.Resize(256, 256),
            A.HorizontalFlip(p=0.5),
    ...
    elif task_name == "Mosmed":  # CT scans
        return A.Compose([
            A.Resize(256, 256),
            A.HorizontalFlip(p=0.5),
    ...
    elif task_name == "Kvasir":  # endoscopy
        return A.Compose([
            A.Resize(256, 256),
            A.HorizontalFlip(p=0.5),
...
def plot_iou_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/sslddmscmexo_wbg.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/sslddmscmexo_nobg.png")
    plt.close()
...
def plot_proxy_mse_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/sslddmscmexo_proxy_mse.png")
    plt.close()
...
def train_local(loader, FE, SS, BE,
    ...
    for ep in range(LOCAL_EPOCHS):
        ...
        print(
            f"[Sched] ep={ep + 1}/{LOCAL_EPOCHS} | ?_proxy={lam_proxy:.3g}, ?_diff={lam_diff:.3g}, ?_KLu={lam_klu:.3g}, ?_KLz={lam_klz:.3g}")
        ...
        for batch in tqdm(loader, leave=False):
            ...
            diff_loss1, x1_hat = diffusion_noise_loss(denoiser1, x1, t_vec, z_causal_1.detach(), alpha_cum,
                                                      want_grad_x0=True)
            ...
            diff_loss2, x2_hat = diffusion_noise_loss(denoiser2, x2, t_vec, z_causal_2.detach(), alpha_cum,
                                                      want_grad_x0=True)
...
def main():
    ...
    for r in range(COMM_ROUNDS):
        ...
        global_SS.load_state_dict(
            causal_invariant_fusion(local_SS, proxy_mse_scores, client_sizes, val_losses, min_fraction=0.05, blend_equal=0.3))
        global_exo1.load_state_dict(
            causal_invariant_fusion(local_exo1, proxy_mse_scores, client_sizes, val_losses, min_fraction=0.05, blend_equal=0.3))
        global_exo2.load_state_dict(
            causal_invariant_fusion(local_exo2, proxy_mse_scores, client_sizes, val_losses, min_fraction=0.05, blend_equal=0.3))
        global_den1.load_state_dict(
            causal_invariant_fusion(local_den1, proxy_mse_scores, client_sizes, val_losses, min_fraction=0.05, blend_equal=0.3))
        global_den2.load_state_dict(
            causal_invariant_fusion(local_den2, proxy_mse_scores, client_sizes, val_losses, min_fraction=0.05, blend_equal=0.3))
        ...
        for i in range(NUM_CLIENTS):
            ...
            val_loader = get_loader(os.path.join(task["path"], f"client{i + 1}/val_imgs"),
                                    ...
                                    ds_class, val_tf, with_names=True, shuffle=False, fraction=0.01)
            loss_fn = ComboLoss(num_classes)
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_SS.state_dict(), "BestModels/MTS5_best_SS_all.pth")
            torch.save(global_exo1.state_dict(), "BestModels/MTS5_best_exo1_all.pth")
            torch.save(global_exo2.state_dict(), "BestModels/MTS5_best_exo2_all.pth")
            torch.save(global_scm1.state_dict(), "BestModels/MTS5_best_scm1_all.pth")
            torch.save(global_scm2.state_dict(), "BestModels/MTS5_best_scm2_all.pth")
            torch.save(global_den1.state_dict(), "BestModels/MTS5_best_den1_all.pth")
            torch.save(global_den2.state_dict(), "BestModels/MTS5_best_den2_all.pth")
