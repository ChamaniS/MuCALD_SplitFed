...
import sys
output_file = "/lustre06/project/6008975/csj5/causalenv/mucald_final_swin.txt"
...
class SwinConfig:
    ...
    MODEL_SWIN_EMBED_DIM = 32    
    MODEL_SWIN_PATCH_NORM = True
    MODEL_DROP_RATE = 0.0
    MODEL_DROP_PATH_RATE = 0.1
    MODEL_SWIN_DEPTHS = [2, 2, 2, 2]   
    MODEL_SWIN_NUM_HEADS = [2, 4, 8, 16]
    MODEL_SWIN_WINDOW_SIZE = 8
...
patch_grid = swin_cfg.DATA_IMG_SIZE // swin_cfg.MODEL_SWIN_PATCH_SIZE
if swin_cfg.DATA_IMG_SIZE % swin_cfg.MODEL_SWIN_PATCH_SIZE != 0:
    raise ValueError(f"DATA_IMG_SIZE ({swin_cfg.DATA_IMG_SIZE}) must be divisible by MODEL_SWIN_PATCH_SIZE ({swin_cfg.MODEL_SWIN_PATCH_SIZE}).")
if patch_grid % swin_cfg.MODEL_SWIN_WINDOW_SIZE != 0:
    raise ValueError(
        f"Patch grid size = DATA_IMG_SIZE/PATCH_SIZE = {patch_grid} is not divisible by MODEL_SWIN_WINDOW_SIZE = {swin_cfg.MODEL_SWIN_WINDOW_SIZE}. "
        "Choose a window size that divides the patch grid (e.g. 8,4,2,16...)."
    )
...
def tokens_to_map(x_tokens, img_size=swin_cfg.DATA_IMG_SIZE, patch_size=swin_cfg.MODEL_SWIN_PATCH_SIZE):
    """
    Robust conversion from tokens -> spatial map.

    Accepts tokens in either shape:
      - (B, L, C)  where L = H'*W', C = embed_dim  (common Swin output)
      - (B, C, L)  where C = embed_dim, L = H'*W'  (some implementations)

    Returns spatial map (B, C, H', W') where H' = W' = sqrt(L).
    Raises a clear error if L isn't a perfect square.
    """
    if x_tokens.dim() != 3:
        raise ValueError(f"tokens_to_map: expected 3D tensor, got shape {tuple(x_tokens.shape)}")

    B, a, b = x_tokens.shape

    embed = swin_cfg.MODEL_SWIN_EMBED_DIM
    ...
    if a == embed:
        ...
        if side * side != L:
            raise ValueError(f"tokens_to_map: token length L={L} is not a perfect square (cannot reshape to square map)")

        x = x_tokens.permute(0, 2, 1).contiguous().view(B, side, side, C).permute(0, 3, 1, 2).contiguous()
        return x
    else:
        ...
        if side * side != L:
            ...
            expected_L = (img_size // patch_size) ** 2
            if L == expected_L:
                side = img_size // patch_size
            else:
                raise ValueError(f"tokens_to_map: token length L={L} is not a perfect square (cannot reshape to square map)")
        x = x_tokens.view(B, side, side, C).permute(0, 3, 1, 2).contiguous()
        return x


def map_to_tokens(x_map):
    """
    Robust conversion from spatial map -> tokens.

    Accepts map in either shape:
      - (B, C, H, W)  normal conv map
      - (B, H, W, C)  some accidental layouts

    Returns tokens as (B, L, C) with L = H*W.
    """
    if x_map.dim() == 4:
        B, d1, d2, d3 = x_map.shape

        if d1 == swin_cfg.MODEL_SWIN_EMBED_DIM:
            B, C, H, W = x_map.shape
            L = H * W
            return x_map.permute(0, 2, 3, 1).contiguous().view(B, L, C)

        elif d3 == swin_cfg.MODEL_SWIN_EMBED_DIM:
            B, H, W, C = x_map.shape
            L = H * W
            return x_map.contiguous().view(B, L, C)
        else:
        ...
        raise ValueError(f"map_to_tokens: expected 4D tensor, got shape {tuple(x_map.shape)}")


def fe_out_to_map(fe_out):
    """
    Convert FE output (tokens or map) to a conv-map (B,C,H,W) for downstream conv modules.
    """
    if fe_out.dim() == 3:
        return tokens_to_map(fe_out)
    elif fe_out.dim() == 4:
        return fe_out
    else:
        raise ValueError(f"fe_out_to_map: expected 3D or 4D tensor, got {tuple(fe_out.shape)}")


def ss_forward(SS, x_in):
    """
    Safely forward through SS (server split). Accepts x_in as map (B,C,H,W) or tokens (B,L,C).
    Ensures the output is a map (B,C,H,W).
    """

    if x_in.dim() == 4:
        toks = map_to_tokens(x_in)
        out = SS(toks)
    elif x_in.dim() == 3:
        out = SS(x_in)
    else:
        raise ValueError(f"ss_forward: unexpected input shape {tuple(x_in.shape)}")


    if out.dim() == 3:
        return tokens_to_map(out)
    elif out.dim() == 4:
        return out
    else:
        raise ValueError(f"ss_forward: unexpected SS output shape {tuple(out.shape)}")
...
def evaluate(loader, FE, SS, BE, denoiser1, denoiser2, exo1, scm1, exo2, scm2,
    ...
    """
    Corrected evaluate():
      - robust FE/SS handling (tokens <-> maps)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            fe_out = FE(data)
            x1 = fe_out_to_map(fe_out)  # (B,C,H,W)
...
def save_test_images(loader, FE, SS, BE, denoiser1, denoiser2, exo1, scm1, exo2, scm2,
                     num_classes, out_dir_root="Outputs/swin_mucald_test_preds", cid=1):
    """
    ...
    def tokens_to_map_local(x_tokens):
        return tokens_to_map(x_tokens)
...
def plot_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/mucald_swin_wb.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/mucald_swin_nb.png")
    plt.close()
...
def main():
    ...
    global_SS = SwinUNet_SS(swin_cfg).to(DEVICE)
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            if global_exo1 is None:
                global_exo_template1 = ExogenousEncoder(in_channels=swin_cfg.MODEL_SWIN_EMBED_DIM, node_dims=node_dims, variational=True).to(DEVICE)
                global_exo_template2 = ExogenousEncoder(in_channels=swin_cfg.MODEL_SWIN_EMBED_DIM, node_dims=node_dims, variational=True).to(DEVICE)
                ...
                global_den_template1 = initialize_conditional_denoiser(swin_cfg.MODEL_SWIN_EMBED_DIM, cond_dim, 128, DEVICE)
                global_den_template2 = initialize_conditional_denoiser(swin_cfg.MODEL_SWIN_EMBED_DIM, cond_dim, 128, DEVICE)
            ...
            if client_FE[i] is None:
                FE = SwinUNet_FE(swin_cfg).to(DEVICE)
                BE = SwinUNet_BE(swin_cfg, num_classes=num_classes).to(DEVICE)
            ...
            if client_dom1[i] is None:
                dom_disc1, dom_disc2 = DomainDiscriminator(in_dim=swin_cfg.MODEL_SWIN_EMBED_DIM).to(DEVICE), DomainDiscriminator(in_dim=swin_cfg.MODEL_SWIN_EMBED_DIM).to(DEVICE)
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_SS.state_dict(), "BestModels/MTS5_best_swin_SS.pth")
            torch.save(global_exo1.state_dict(), "BestModels/MTS5_best_swin_exo1.pth")
            torch.save(global_exo2.state_dict(), "BestModels/MTS5_best_swin_exo2.pth")
            torch.save(global_scm1.state_dict(), "BestModels/MTS5_best_swin_scm1.pth")
            torch.save(global_scm2.state_dict(), "BestModels/MTS5_best_swin_scm2.pth")
            torch.save(global_den1.state_dict(), "BestModels/MTS5_best_swin_den1.pth")
            torch.save(global_den2.state_dict(), "BestModels/MTS5_best_swin_den2.pth")
        ...
        for i in range(NUM_CLIENTS):
            ...
            save_test_images(
                ...
                out_dir_root="Outputs/swin_mucald_test_preds",
                cid=i + 1
            )
