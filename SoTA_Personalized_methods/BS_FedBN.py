# Multi-task SplitFed Training Script (FedBN integrated)
import os
...
import sys
output_file = "/lustre06/project/6008975/csj5/causalenv/BS_FedBN.txt"
sys.stdout = open(output_file, "w")

# FedBN paper reference (keeps BN local / do not aggregate BN params).
# See: FedBN: Federated Learning on Non-IID Features via Local Batch Normalization.
# Citation token for the uploaded paper file: :contentReference[oaicite:1]{index=1}
...
def is_bn_param(key):
    """Return True if state_dict key corresponds to BatchNorm parameters/statistics."""
    # common BN substrings: 'bn', 'batchnorm', 'running_mean', 'running_var', 'num_batches_tracked'
    k = key.lower()
    return ('bn' in k) or ('batchnorm' in k) or ('running_mean' in k) or ('running_var' in k) or ('num_batches_tracked' in k)

def average_models_weighted(models, weights, global_model_state=None):
    """
    Average state_dicts of models but exclude BN parameters from averaging (FedBN).
    - models: list of nn.Module objects (clients' local SS models)
    - weights: list of floats summing to 1 (normalized weights)
    - global_model_state: state_dict of global SS to take BN params from (if provided).
      If not provided, BN params are taken from the first model.
    Returns an averaged state_dict where BN params are preserved (not averaged).
    """
    n = len(models)
    if n == 0:
        return None
    ...
    for k in avg_sd.keys():
        if is_bn_param(k):
            # keep BN params from global_model_state if provided, else from first model
            if global_model_state is not None and k in global_model_state:
                avg_sd[k] = global_model_state[k].clone()
            else:
                avg_sd[k] = sds[0][k].clone()
        else:
            # weighted average across models (must exist in each sd)
            val = None
            for i in range(n):
                part = sds[i][k].float() if isinstance(sds[i][k], torch.Tensor) else torch.tensor(sds[i][k])
                if val is None:
                    val = weights[i] * part
                else:
                    val = val + weights[i] * part
            # ensure dtype matches original (convert back)
            avg_sd[k] = val.type(sds[0][k].dtype)
    return avg_sd

def merge_global_nonbn_with_local_bn(global_state, local_state):
    """
    Produce a merged state_dict: global non-BN params + local BN params.
    - global_state: state_dict of global SS (non-BN)
    - local_state: state_dict of client's SS (local BN)
    """
...
def plot_iou_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/FedBN_MTS1BS_iou_wbg.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/FedBN_MTS1BS_iou_nobg.png")
    plt.close()
...
def main():
    ...
    # initialize global_SS weights if you want (kept default)
    # Important: global_SS BN params will be considered "server side" placeholders;
    # but FedBN keeps clients' BN local.

    for r in range(COMM_ROUNDS):
        print(f"\n[Comm Round {r+1}/{COMM_ROUNDS}]")
        local_FE_list, local_SS_list = [], []
        weights, total_sz = [], 0

        client_FEs = []
        client_BEs = []
        client_SS = []  # keep each client's trained SS (with local BN) for FedBN merging later
        ...
        for i in range(NUM_CLIENTS):
            ...
            # initialize client's SS with global non-BN parameters but preserve local BN in client's SS
            # We load full global state first and then client will update BN locally during its training.
            ...
            # local training (clients will update BN params locally)
            ...
            local_FE_list.append(FE)
            local_SS_list.append(SS)      # collect client's local SS (contains local BN)
            client_BEs.append(BE)
            client_SS.append(SS)         # store per-client SS model for merging/evaluation
        ...
        # Average server splits (SS) but exclude BN params (FedBN). Use global_SS state for BN preservation.
        averaged_sd = average_models_weighted(local_SS_list, norm_weights, global_model_state=global_SS.state_dict())
        if averaged_sd is not None:
            global_SS.load_state_dict(averaged_sd)

        # After obtaining new global non-BN parameters, when performing validation per-client, we
        # must evaluate with global non-BN parameters combined with each client's local BN parameters.
        ...
        for i in range(NUM_CLIENTS):
            ...
            # create a merged SS for evaluation: global non-BN + client local BN
            merged_sd = merge_global_nonbn_with_local_bn(global_SS.state_dict(), client_SS[i].state_dict())
            eval_SS = UNET_server(in_channels=32).to(DEVICE)
            eval_SS.load_state_dict(merged_sd)
            # use client's FE and BE (trained locally) with merged SS for validation
            val_loss, val_acc, val_iou_wb, val_iou_nb = evaluate(val_loader, client_FEs[i], eval_SS, client_BEs[i], loss_fn, num_classes)
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_SS.state_dict(), "BestModels/FedBN_best_SS.pth")
            for idx in range(NUM_CLIENTS):
                torch.save(client_FEs[idx].state_dict(), f"BestModels/FedBN_best_FE_client{idx + 1}.pth")
                torch.save(client_BEs[idx].state_dict(), f"BestModels/FedBN_best_BE_client{idx + 1}.pth")
                # save BN of client's SS as well (FedBN keeps BN local)
                torch.save(client_SS[idx].state_dict(), f"BestModels/FedBN_best_SS_client{idx + 1}_localBN.pth")
        ...
        for i in range(NUM_CLIENTS):
            ...
            # create merged SS for testing: global non-BN + client local BN
            merged_sd = merge_global_nonbn_with_local_bn(global_SS.state_dict(), client_SS[i].state_dict())
            test_SS = UNET_server(in_channels=32).to(DEVICE)
            test_SS.load_state_dict(merged_sd)

            # Fixed save_dir per client so files are overwritten each round
            save_dir = os.path.join("Outputs/test_preds_FedBN_BS", f"client{i+1}_preds")
