...
import sys
output_file = "XXXXX/xxxx/BS_FedPer.txt"
...
def plot_iou_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/BSFedPer_iou_wbg.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/BSFedPer_iou_nobg.png")
    plt.close()
...
def main():
    ...
    # If using FedPer, create a global_FE which will be the shared base (sent to clients every round).
    # This follows the "base layers shared, personalization layers local" idea from FedPer.
    # If USE_FEDPER is False, this global_FE will be unused and we retain original behavior.
    global_FE = UNET_FE(in_channels=3).to(DEVICE) if USE_FEDPER else None
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            # If USE_FEDPER: initialize client FE from server's global_FE (FedPer base layers shared).
            if USE_FEDPER and global_FE is not None:
                # load weights of shared base into client's FE
                FE.load_state_dict(global_FE.state_dict())
            # Otherwise, for backwards compatibility with original code, keep SS as the shared entity
        ...
        # FedPer behavior: aggregate client base layers (FE) into global_FE
        if USE_FEDPER:
            # average client FEs weighted by data size -> update global_FE
            global_FE.load_state_dict(average_models_weighted(client_FEs, norm_weights))
        else:
            # original behavior: aggregate server-side SS into global_SS (kept for compatibility)
        ...
        for i in range(NUM_CLIENTS):
            ...
            val_loader = get_loader(os.path.join(path, f"client{i + 1}/val_imgs"),
                                    ...
                                    ds_class, val_tf)
            loss_fn = ComboLoss(num_classes)
            # NOTE: validation uses the client's FE (which is local) and the shared base if FedPer is active.
            # Use global_FE as the shared base during validation if FedPer; otherwise use global_SS as before.
            if USE_FEDPER and global_FE is not None:
                # Use client's FE and client's SS and client's BE for per-client validation
                # We pass client_FEs[i] and client_BEs[i], and local server-side SS is not aggregated in FedPer,
                # but we still have local_SS[i] that was used in training, so we validate with that.
                val_loss, val_acc, val_iou_wb, val_iou_nb = evaluate(val_loader, client_FEs[i], local_SS[i], client_BEs[i], loss_fn, num_classes)
            else:
        ...
        if avg_val_loss < best_loss:
            ...
            # Save whichever global/shared model is active. For FedPer save global_FE too.
            if USE_FEDPER and global_FE is not None:
                torch.save(global_FE.state_dict(), "BestModels/BSFedPerbest_FE.pth")
            torch.save(global_SS.state_dict(), "BestModels/BSFedPerbest_SS.pth")
            for i in range(NUM_CLIENTS):
                torch.save(client_FEs[i].state_dict(), f"BestModels/BSFedPerbest_FE_client{i + 1}.pth")
                torch.save(client_BEs[i].state_dict(), f"BestModels/BSFedPerbest_BE_client{i + 1}.pth")
        ...
        for i in range(NUM_CLIENTS):
            ...
            save_dir = os.path.join("Outputs/BS_FedPer_test_preds", f"client{i+1}_preds")
            # For evaluation we use the client's FE (local) and their SS and BE. If FedPer is used,
            # global_FE has been sent to clients prior to training and client_FEs[i] reflect client base.
