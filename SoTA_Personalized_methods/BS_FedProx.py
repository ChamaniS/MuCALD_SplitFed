# Multi-task SplitFed Training Script with FedProx integrated
import os
...
import sys
output_file = "/lustre06/project/6008975/csj5/causalenv/BS_FedProx.txt"
...
FEDPROX_MU = 0.01  # adjust as needed (0.0 disables the proximal term)
...
def compute_fedprox_prox_term(local_models_params, global_models_params, mu):
    """
    local_models_params and global_models_params are lists of iterables of tensors
    (for example zipped lists of parameters for FE and SS). Return scalar tensor prox term.
    """
    prox = 0.0
    # local_models_params and global_models_params are lists of parameter iterables
    for local_iter, global_iter in zip(local_models_params, global_models_params):
        for lp, gp in zip(local_iter, global_iter):
            prox = prox + torch.sum((lp - gp) ** 2)
    prox = 0.5 * mu * prox
    return prox

def train_local(loader, FE, SS, BE, opt, loss_fn, cid, num_classes, global_FE, global_SS, mu=0.0):
    ...
    for ep in range(LOCAL_EPOCHS):
        ...
        for data, target in tqdm(loader, leave=False):
            ...
            # FedProx proximal term (FE + SS)
            if mu > 0.0:
                # collect parameter iterables
                local_iters = [FE.parameters(), SS.parameters()]
                global_iters = [global_FE.parameters(), global_SS.parameters()]
                prox_term = compute_fedprox_prox_term(local_iters, global_iters, mu)
                loss = loss + prox_term
            ...
            for i in range(num_classes):
                iou_c[i] += ious[i]
        acc = 100.0 * tcorrect / max(1, len(loader.dataset))
        avg_iou = [v / max(1, len(loader.dataset)) for v in iou_c]
...
def evaluate(loader, FE, SS, BE, loss_fn, num_classes):
    ...
    avg_iou = [v / max(1, N) for v in iou_c]
    ...
    return total_loss / max(1, N), acc, avg_iou, avg_iou[1:]

# ... (unchanged metric helper functions) ...
...
def plot_iou_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/BS_FedProx_wbg.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/BS_FedProx_nobg.png")
    plt.close()
...
def main():
    ...
    # initialize global shared models (FE and SS)
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            # instantiate local models, load global params for FE and SS (FedProx uses global snapshot)
            FE = UNET_FE(in_channels=3).to(DEVICE)
            FE.load_state_dict(global_FE.state_dict())

            SS = UNET_server(in_channels=32).to(DEVICE)
            SS.load_state_dict(global_SS.state_dict())

            BE = UNET_BE(out_channels=num_classes).to(DEVICE)  # personalized BE (task-specific)

            # optimizer includes FE and SS importantly, and BE as well (BE personalized)
            ...
            # LOCAL TRAINING with FedProx proximal term referencing global_FE and global_SS
            train_local(train_loader, FE, SS, BE, opt, loss_fn, i + 1, num_classes, global_FE, global_SS, mu=FEDPROX_MU)
        ...
        # aggregate and update global FE and SS (weighted average)
        global_FE.load_state_dict(average_models_weighted(client_FEs, norm_weights))
        ...
        for i in range(NUM_CLIENTS):
            ...
            val_loader = get_loader(os.path.join(path, f"client{i + 1}/val_imgs"),
                                    ...
                                    ds_class, val_tf)
            loss_fn = ComboLoss(num_classes)
            # NOTE: evaluate using client's FE and BE but the aggregated global SS (server split)
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_SS.state_dict(), "BestModels/Fedprox_best_SS.pth")
            torch.save(global_FE.state_dict(), "BestModels/Fedprox_best_FE.pth")
            for i in range(NUM_CLIENTS):
                torch.save(client_FEs[i].state_dict(), f"BestModels/Fedprox_best_FE_client{i + 1}.pth")
                torch.save(client_BEs[i].state_dict(), f"BestModels/Fedprox_best_BE_client{i + 1}.pth")
        ...
        for i in range(NUM_CLIENTS):
            ...
            save_dir = os.path.join("Outputs/test_preds_Fedprox", f"client{i+1}_preds")
