# Multi-task SplitFed + SCAFFOLD integration (copy-paste)
import os
...
def sd_zero_like(sd, device=None):
    out = {}
    for k, v in sd.items():
...
def _ensure_same_device(tensor, device):
...
def sd_to_device(sd, device):
    """Move state-dict to device (string or torch.device)."""
    return {k: v.to(device) for k, v in sd.items()}

def sd_clone(sd, device=None):
    """Clone and detach tensors; optionally move to device."""
    out = {}
    for k, v in sd.items():
        t = v.clone().detach()
        if device is not None:
...
def sd_average(list_of_sds, weights=None):
    """Average list of state-dicts. Returns result on the device of the first sd's tensors."""
    n = len(list_of_sds)
    if weights is None:
        weights = [1.0 / n] * n
    out = sd_zero_like(list_of_sds[0])
    first_dev = next(iter(list_of_sds[0].values())).device
    # accumulate
    for w, sd in zip(weights, list_of_sds):
        for k in sd.keys():
            sk = sd[k]
            if sk.device != first_dev:
                sk = sk.to(first_dev)
            out[k] = out[k] + sk * w
    return out
# --- end helpers ---
...
def train_local_scaffold(loader, FE, SS, BE, opt, loss_fn, cid, num_classes,
                         ci_sd, c_sd, local_lr=ETA_L):
    FE.train(); SS.train(); BE.train()
    step_count = 0
    # keep initial copy of server SS (x)
    x_sd = sd_clone(SS.state_dict())  # note: SS already has been loaded with global_x prior to call

    ss_param_map = {name: param for name, param in SS.named_parameters()}

    # ensure ci and c are on DEVICE and same dtype
    ci_sd_dev = sd_to_device(ci_sd, DEVICE)
    c_sd_dev = sd_to_device(c_sd, DEVICE)
    ...
    for ep in range(LOCAL_EPOCHS):
        ...
        for data, target in tqdm(loader, leave=False):
            ...
            for name, param in ss_param_map.items():
                if name in c_sd_dev:
                    corr = c_sd_dev[name] - ci_sd_dev[name]
                    if param.grad is None:
                        param.grad = corr.clone().detach()
                    else:
                        param.grad = param.grad + corr
                else:
                    matched = None
                    for k in c_sd_dev.keys():
                        if k.endswith(name):
                            matched = k
                            break
                    if matched is not None:
                        corr = c_sd_dev[matched] - ci_sd_dev[matched]
                        if param.grad is None:
                            param.grad = corr.clone().detach()
                        else:
                            param.grad = param.grad + corr
    ...
    yi_sd = sd_clone(SS.state_dict())
    K = max(1, step_count)
    x_minus_yi = sd_sub(x_sd, yi_sd)
    factor = 1.0 / (K * local_lr)
    scaled = sd_scale(x_minus_yi, factor)
    c_plus_i = {}
    for k in ci_sd.keys():
        c_plus_i[k] = ci_sd[k].to(DEVICE) - c_sd[k].to(DEVICE) + scaled[k].to(DEVICE)

    acc = 100.0 * tcorrect / len(loader.dataset)
    avg_iou = [v / len(loader.dataset) for v in iou_c]
    print(f"Client {cid} | Train Steps: {step_count} | Train Loss: {tloss:.4f} | Train Acc: {acc:.2f}% | "
          f"Train IoU w/bg: {sum(avg_iou)/num_classes:.4f} | "
          f"Train IoU no/bg: {sum(avg_iou[1:])/(num_classes-1):.4f}")

    return yi_sd, c_plus_i, step_count
...
def plot_iou_curves(round_num):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/BSSCAFFOLD_iou_wbg.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Outputs/BSSCAFFOLD_nobg.png")
    plt.close()

# ---------- Metric and evaluation functions (complete) ----------
...
# ---------- End metric and evaluation functions ----------
...
def main():
    ...
    global_c_sd = sd_zero_like(global_SS.state_dict())
    client_cis = [sd_zero_like(global_SS.state_dict()) for _ in range(NUM_CLIENTS)]
    ...
    for r in range(COMM_ROUNDS):
        ...
        for i in range(NUM_CLIENTS):
            ...
            opt = optim.AdamW(list(FE.parameters()) + list(SS.parameters()) + list(BE.parameters()), lr=ETA_L)
            ...
            yi_sd, c_plus_i_sd, steps = train_local_scaffold(train_loader, FE, SS, BE, opt, loss_fn, i + 1, num_classes,
                                                            client_cis[i], global_c_sd, local_lr=ETA_L)

            local_Ss_state_dicts.append(yi_sd)
            c_plus_list.append(c_plus_i_sd)

            # local eval with client SS (trained copy)
        ...
        S = NUM_CLIENTS
        deltas_y = [sd_sub(local_Ss_state_dicts[i], global_SS.state_dict()) for i in range(len(local_Ss_state_dicts))]
        avg_delta_y = sd_average(deltas_y)

        new_global_sd = {}
        cur_global_sd = global_SS.state_dict()
        for k in cur_global_sd.keys():
            new_global_sd[k] = cur_global_sd[k] + ETA_G * avg_delta_y[k].to(cur_global_sd[k].device)
        global_SS.load_state_dict(new_global_sd)

        delta_cs = []
        for i in range(NUM_CLIENTS):
            delta_c_i = sd_sub(c_plus_list[i], client_cis[i])
            delta_cs.append(delta_c_i)
        avg_delta_c = sd_average(delta_cs)

        for k in global_c_sd.keys():
            global_c_sd[k] = global_c_sd[k] + avg_delta_c[k].to(global_c_sd[k].device)

        for i in range(NUM_CLIENTS):
            client_cis[i] = {k: c_plus_list[i][k].detach().cpu() for k in c_plus_list[i].keys()}
        ...
        if avg_val_loss < best_loss:
            ...
            torch.save(global_SS.state_dict(), "BestModels/BSSCAFFOLD_best_SS.pth")
            for i in range(NUM_CLIENTS):
                torch.save(client_FEs[i].state_dict(), f"BestModels/BSSCAFFOLD_best_FE_client{i + 1}.pth")
                torch.save(client_BEs[i].state_dict(), f"BestModels/BSSCAFFOLD_best_BE_client{i + 1}.pth")
        ...
        for i in range(NUM_CLIENTS):
            ...
            save_dir = os.path.join("Outputs/test_preds_BSSCAFFOLD", f"client{i+1}_preds")
