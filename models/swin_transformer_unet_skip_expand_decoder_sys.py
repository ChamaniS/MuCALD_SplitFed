        ...
        # build decoder layers
            ...
            for i_layer in range(self.num_layers):
                ...
                if i_layer == 0:
                    ...
                    F_g = int(embed_dim * 2 ** (self.num_layers - 1 - i_layer))
                    F_l = F_g
                    F_int = F_g // 2
                    attn_gate = AttentionGate(F_g, F_l, F_int)
                    self.attn_gates.append(attn_gate)

                self.layers_up.append(layer_up)
                self.concat_back_dim.append(concat_linear)

            # Pad attn_gates list to ensure it has same length as layers_up
            while len(self.attn_gates) < len(self.layers_up):
                self.attn_gates.append(None)
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                skip = x_downsample[self.num_layers - 1 - inx]
                if self.attn_gates[inx] is not None:
                    skip = self.attn_gates[inx](g=x, x=skip)
